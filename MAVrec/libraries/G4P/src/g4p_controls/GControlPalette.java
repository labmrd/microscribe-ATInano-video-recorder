 ...
 * over it one or more G4P controls will appear in a straight line radially 
 * from the tab, this is called the palette. <br>
 ...
 * The preferred alignment will always be used provided it fits on the 
 * screen, if it doesn't then the control will use the opposite alignment
 * if that can be displayed fully. <br>
 * 
...
public class GControlPalette extends GAbstractControl {
	...
	/**
	 * Recalculate the control positions after a change of palette alignment
	 * or when a control is added to or removed from the palette.
	 */
	 ...
	 * If the palette does not fit the screen then it will
	 * not be shown until the control is moved to a position it
	 * can be seen.
	...
	/**
	 * Will change the current alignment to the preferred alignment
	 * if it can be displayed fully on screen. If not it will attempt 
	 * to use the opposite alignment.
	 */
	...
	/**
	 * For any valid alignment it returns the alignment on the opposite
	 * side of the tab (icon).
	 * 
	...
	/**
	 * Attempts to add a control to the end of the palette. If the position is invalid
	...
	/**
	 * Attempts to insert a control at a given index position. If the position is invalid
	 * or the control is a duplicate then it is silently ignored.
	 * 
	...
	/**
	 * Attempts to add one or more controls to the end of the palette. If the position 
	 * is invalid or the control is a duplicate then it is silently ignored.
	 * 
	...
	public void addControls(GAbstractControl... controls){
		 ...
		 if(success) updateControlPositions();
	 }
	 ...
	 */
	 ...
	 /**
	  * Removes a control from the palette. If successful the control will
	  * unlinked from the palette and made invisible but not disposed of. <br>
	  * If the control is no longer needed then use the <pre>control.dispose()</pre>
	  ...
	  * The index will be 0 (zero) for the control nearest the tab no matter
	  * what alignment is in effect. <br>
	  * 
	 ...
	 /**
	  * See if the control fits inside the window based if
	  * it's current alignment is the same as passed in
	  * the parameter. <br> 
	  * 
	  * @param align
	  * @return
	 ...
	 private void calcHotSpots(){
		 hotspots = new HotSpot[]{
				 new HSrect(TAB, -1, -1, width + 1, height + 1),					// tab icon area
	 ...
	 /**
	  * Set the border weight and the corner radii to apply. The border must be at least 1 pixel 
	  * so you can see the palette area limits. A radii value of 0 will result in square corners.
	  * 
	 ...
	 public void mouseEvent(MouseEvent event){
		 ...
		 case MouseEvent.CLICK:
			 if(focusIsWith == this){
				 rVisible = false;
				 //				fireEvent(this, GEvent.EXPANDED); // we could have an event if the tab is clicked on
				 bufferInvalid = true;
				 beingDragged = false;
