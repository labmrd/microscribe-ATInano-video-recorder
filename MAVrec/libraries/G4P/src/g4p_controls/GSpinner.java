...
/**
 * The spinner control can be used to enter any positive or negative integer value either 
 * through the keyboard, the up/down spinner buttons or the mouse wheel when over the control. <br>
 * 
 * After creating the spinner the user can set the initial value to display and the minimum and 
 * maximum permitted values. The user also specifies the increment/decrement value to apply when
 * using the spinner buttons or mouse wheel to change the value. <br>
 * 
 * When entering numbers from the keyboard the - key can be pressed at any time to negate the 
 * current value.
 * 
 * @author Peter Lager
 *
...
public class GSpinner extends GEditableTextControl {
	...
	public GSpinner(PApplet theApplet, float p0, float p1, float p2, float p3, float buttonWidth) {
		...
		addControl(btnDec, tw + 2, 1 + th/2, 0);
	 ...
	 * The initial value will be constrained to the range &ge; low and &le; high. <br>
	 * The increment / decrement value will not be allowed to exceed the range i.e. high - low
	 *  
	...
	/**
	 * Decrement the current value. It will be constrained to the
	...
	/**
	 * Increment the current value. It will be constrained to the
	 * current range limits
	 * @return true if the value changes
	 */
	public boolean inc() {
	...
	/**
	 * Sets the current value to be displayed. It will be constrained to the 
	 * current range limits.
...
//					dec();
	...
	protected void keyTypedProcess(int keyCode, char keyChar, boolean shiftDown, boolean ctrlDown){
		...
		if(keyCharIdx >= 0){
			...
			System.out.println("Char at " + minusIdx + " = '" + stext.getPlainText().charAt(minusIdx) + "'   Len = " + stext.length());
