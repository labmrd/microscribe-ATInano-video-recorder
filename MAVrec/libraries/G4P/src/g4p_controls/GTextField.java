 ...
 * 
 * As well as text this control will accept numeric input either integer or decimal 
 * within a user specified range. Invalid user input is highlighted in the control
 ...
 * 
 * From V4.3.3
 * The text field can be used to validate numeric input within a user defined range. The input
 * can be either integer {@code int} or decimal {@code float}, using the {@code setNumeric(,,)} 
 * method. If all three parameters are integers then only integer values in the range are valid, 
 * if <em>any</em> of the parameters is a {@code float} then any decimal value in the range is valid. <br>
 * 
...
public class GTextField extends GEditableTextControl {
	...
	/**
	 * Will cause the text field to show when an invalid integer has been 
	 * entered into the field. <br>
	 * An invalid integer is any text that cannot be converted into an integer
	 * or is outside the range indicated by the first two parameters. <br>
	...
	/**
	 * Will cause the text field to show when an invalid float has been 
	 * entered into the field. <br>
	 * An invalid float is any text that cannot be converted into a float
	 * or is outside the range indicated by the first two parameters. <br>
	 * If {@code getValueF()} is called then it will return the current valid
	 * float or if invalid the value passed in the third parameter. <br>
	 ...
	 * @param default_value the value to be returned by {@code getValueF()}for invalid user input
	 */
	 ...
	 * G4P.DECIMAL or G4P.EXPONENT and the accepted range will be based on
	 * range of valid values for the Java data types {@code int} or 
	 * {@code float}. <br>
	 * if the parameter is null or missing then it cancels the filter and all
	 * text values are considered valid.
	 * 
	...
	/**
	 * If the field is displaying a valid integer within the specified range return it. 
	 * Otherwise the default int value.
	 */
	...
	/**
	 * If the field is displaying a valid float within the specified range return it. 
	 * Otherwise the default float value.
	 */
